...
| 14 | `.watchdogTimeout` | Pipeline watchdog (30s prod, 0.2s tests) fires on any stuck transition. | SayMoore error | (default formatter) | Forced reset → `.idle`. | No | Automatic — state machine reset; user re-records. **Gap:** no bespoke copy in `message(for:)`. |
